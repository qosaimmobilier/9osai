...
export function ExhibitorProfile() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <h1 className="text-2xl font-bold text-gray-900">الحساب الشخصي</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
        <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
          ...
          <div>
            <p className="font-bold text-gray-900">{profile?.full_name || 'بدون اسم'}</p>
            <p className="text-sm text-gray-500" dir="ltr">{profile?.email}</p>
            <p className="text-xs text-gray-400 mt-1">عارض عقارات</p>
          </div>
        </div>
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center justify-center gap-2 px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-lg transition-colors disabled:opacity-50"
        >
